...
#wordcloud
  ...
  output$downloadwordcloud.pdf<-downloadHandler(
    filename <-function() {paste('wordcloud.pdf')},
    content <-function(file){
      ...
      graph<- wordcloud2(result,
                         size = input$size,
                         shape = input$shape,
                         color = color,
                         backgroundColor = input$backgroundColor)
      file1.html<-saveWidget(graph, "file1.html", selfcontained = F)
      webshot("file1.html", file, delay = 3, vwidth = 1000, vheight = 1000)
